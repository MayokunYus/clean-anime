...
export default function Home() {
  ...
  const search = async (q: string) => {
    ...
    setLoading(true);
    setSelected(null);
    const res = await fetch(`https://api.jikan.moe/v4/anime?q=${encodeURIComponent(q)}&limit=9&sfw=true`);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#fff", fontFamily: "sans-serif", padding: "24px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: "8px", marginBottom: "24px" }}>
        ...
        />
        <button onClick={() => search(query)} style={{ padding: "12px 24px", background: "#ff3d6b", color: "#fff", border: "none", borderRadius: "8px", fontSize: "16px", cursor: "pointer" }}>
          Search
        </button>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: "8px", flexWrap: "wrap", marginBottom: "32px" }}>
        {["Naruto", "One Piece", "Death Note", "Demon Slayer", "Jujutsu Kaisen"].map((t) => (
          <button key={t} onClick={() => { setQuery(t); search(t); }} style={{ padding: "6px 14px", background: "#1a1a2e", border: "1px solid #333", color: "#aaa", borderRadius: "100px", cursor: "pointer", fontSize: "13px" }}>
            {t}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "16px", maxWidth: "900px", margin: "0 auto" }}>
        {results.map((anime) => (
          <div key={anime.mal_id} onClick={() => setSelected(anime)} style={{ background: "#111", border: "1px solid #222", borderRadius: "10px", overflow: "hidden", cursor: "pointer" }}>
